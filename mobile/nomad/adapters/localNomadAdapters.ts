...
export const localNomadSecurityAdapter: NomadSecurityAdapter = { async getSecurityState() { return buildSecurityState(); }, async runSecurityScan() { return buildSecurityState(); }, async activateFreeze(scope: NomadFreezeScope) { const status: NomadFreezeActivity['status'] = scope === 'owner_authority_alert' ? 'alert_sent' : 'active'; freezeActivity = [{ scope, label: freezeLabel(scope), requestedAt: new Date().toISOString(), status }, ...freezeActivity.filter((item) => item.status !== 'active')].slice(0, 5); return buildSecurityState(); }, async clearFreeze() { freezeActivity = freezeActivity.map((item) => item.status === 'active' ? { ...item, status: 'cleared' as const } : item); return buildSecurityState(); } };
